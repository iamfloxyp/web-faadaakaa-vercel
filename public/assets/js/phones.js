  ...
  /* ============================================================
   PHONES PAGE PRODUCT LOADER (Dynamic API Version)
   Loads real products from Faadaakaa backend
   ============================================================ */

// 🔹 API endpoint for Phones and Tablets Category
const PHONES_API =
  "https://api.faadaakaa.com/api/loadproductbycat/phones-tablet/1";
  ...
  fetch(PHONES_API)
    ...
    .then(result => {
      if (result.status === true && Array.isArray(result.data)) {
        ...
        // Inside loadPhones() after successful fetch:
initializePagination(result.data, renderPhones);
      } else {
        console.log("No phones data returned from API");
      }
    })
    .catch(error => {
      console.log("Error loading phones category", error);
    });
}

// 🔹 Render phones using same card style you already designed
function renderPhones(products) {
  ...
  products.forEach(item => {
    ...
    // Handle image EXACTLY like electronics
    const imageURL =
...
// 🔹 Load phones immediately when the page is ready
